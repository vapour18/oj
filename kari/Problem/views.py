...
def showContestProblem(request,c_id,idx): # modified
    ...
    try:
        ...
        return render(request, "newtpl/problem/showContestProblem.html", {'c':c, 'cp_list':cp_list, 'p':p, 'status_query':queryString(problem_index=p.idx), 'tpl':{'sp':True, 'can_manage':can_manage}})
    except Exception as e:
        logger.error(str(e).replace("\n","\t"))
        return render(request, Err.ERROR_PAGE)
